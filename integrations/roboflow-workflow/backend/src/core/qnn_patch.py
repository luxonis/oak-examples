...
2. creates the session on the DSP via `depthai_nodes.runtime.onnx_qnn_session()`
   with fp16 HTP execution, caching the compiled graph (EPContext) so
...
def install() -> bool:
    ...
    (returning False) with `EP=cpu`; otherwise individual sessions use the
    QNN helper and fall back to CPU when the DSP is unavailable.
    """
    global _installed
    ...
    if not _installed:
        ...
        _installed = True
    logger.info("Inference ONNX sessions will prefer the QNN EP when available")
    return True
...
def _try_qnn(path_or_bytes, providers):
    ...
    try:
        session = onnx_qnn_session(
            static_path, fp16=True, fallback_to_cpu=not _strict()
        )
    finally:
