...
class PFMReader:
    ...
    def read(file_path):
        with open(file_path, 'rb') as file:
            ...
            data = np.fromfile(file, dtype=endian + 'f')
            ...
            try:
                ...
                raise ValueError(f"PFM data mismatch: Header says {width}x{height}x{channels}, "
                                 f"but found {data.size} floats.")
...
class StereoDataSample:
    ...
    def __init__(
        ...
        if padding_mode not in [self.PAD_TOP_RIGHT, self.PAD_CENTER]:
            raise ValueError(f"Invalid padding_mode: {padding_mode}. Must be '{self.PAD_TOP_RIGHT}' or '{self.PAD_CENTER}'")
    ...
    def _preprocess(self, debug=False):
        ...
        if self.original_gt is not None:
            ...
            gt_eval, _ = self._resize_pad_safe(self.original_gt, self.eval_size, is_disparity=True)
        ...
        if debug:
            print(f"[DEBUG _preprocess] After resize to inference: shape={l_inf_u8.shape}")
    ...
    def get_eval_images(self, strip_padding=False):
        ...
        if strip_padding:
            ...
            h_end = self.eval_size[0] - pad_bottom if pad_bottom > 0 else self.eval_size[0]
            w_end = self.eval_size[1] - pad_right if pad_right > 0 else self.eval_size[1]
            
            return l_eval[pad_top:h_end, pad_left:w_end], r_eval[pad_top:h_end, pad_left:w_end]
    ...
    def set_predictions(self, disparity, confidence, edge, conf_threshold, edge_threshold, debug=False):
        ...
        h_end = self.inference_size[0] - pad_bottom if pad_bottom > 0 else self.inference_size[0]
        w_end = self.inference_size[1] - pad_right if pad_right > 0 else self.inference_size[1]
        
        if debug:
            print(f"[DEBUG set_predictions] inference_size={self.inference_size}, eval_size={self.eval_size}")
            print(f"[DEBUG set_predictions] meta_step2={self.meta_step2}")
            print(f"[DEBUG set_predictions] crop region: [{pad_top}:{h_end}, {pad_left}:{w_end}]")
            print(f"[DEBUG set_predictions] disparity input shape={disparity.shape}, range=[{disparity.min():.3f}, {disparity.max():.3f}]")
        ...
        if debug:
            print(f"[DEBUG set_predictions] cropped shape={cropped.shape}, cropped_w={cropped_w}")
            print(f"[DEBUG set_predictions] disp scale factor = {self.eval_size[1]} / {cropped_w} = {self.eval_size[1] / cropped_w:.6f}")
        
        disp_eval = cv2.resize(cropped, (self.eval_size[1], self.eval_size[0]), interpolation=cv2.INTER_LINEAR)
        disp_eval = disp_eval * (self.eval_size[1] / cropped_w)
        
        if debug:
            print(f"[DEBUG set_predictions] disp_eval BEFORE filtering: range=[{disp_eval.min():.3f}, {disp_eval.max():.3f}]")

        conf = np.squeeze(confidence)
        cropped = conf[pad_top:h_end, pad_left:w_end]
        conf_eval = cv2.resize(cropped, (self.eval_size[1], self.eval_size[0]), interpolation=cv2.INTER_LINEAR)

        edg = np.squeeze(edge)
        cropped = edg[pad_top:h_end, pad_left:w_end]
        edge_eval = cv2.resize(cropped, (self.eval_size[1], self.eval_size[0]), interpolation=cv2.INTER_LINEAR)
    ...
    def get_predictions(self, target='eval', strip_padding=False):
        ...
        if target == 'eval':
            ...
            cropped_w = cropped.shape[1]
            final_disparity = cv2.resize(cropped, (self.original_size[1], self.original_size[0]), interpolation=cv2.INTER_LINEAR)
            final_disparity = final_disparity * (self.original_size[1] / cropped_w)

            cropped = self._eval_confidence[pad_top:h_end, pad_left:w_end]
            final_confidence = cv2.resize(cropped, (self.original_size[1], self.original_size[0]), interpolation=cv2.INTER_LINEAR)

            cropped = self._eval_edge[pad_top:h_end, pad_left:w_end]
            final_edge = cv2.resize(cropped, (self.original_size[1], self.original_size[0]), interpolation=cv2.INTER_LINEAR)
    ...
    def compute_metrics(self, target='eval', is_legacy_code=False, strip_padding=False, debug=False):
        if self.original_gt is None:
            return {}
        if self._eval_disparity is None:
        ...
        if target == 'eval':
            if strip_padding:
                ...
                if self.border_erase_pixels > 0:
                    ...
                    if debug:
                        print(f"[DEBUG compute_metrics] Applied border_erase({self.border_erase_pixels}) after strip_padding")
            else:
            ...
            cropped = self._eval_disparity[pad_top:h_end, pad_left:w_end]
            cropped_w = cropped.shape[1]
            pred = cv2.resize(cropped, (self.original_size[1], self.original_size[0]), interpolation=cv2.INTER_LINEAR)
        ...
        if debug:
            ...
            print(f"[DEBUG compute_metrics] gt shape={gt.shape}, range=[{gt.min():.3f}, {gt.max():.3f}]")
            print(f"[DEBUG compute_metrics] pred shape={pred.shape}, range=[{pred.min():.3f}, {pred.max():.3f}]")
            print(f"[DEBUG compute_metrics] gt>0 count={(gt > 0).sum()}, pred>0 count={(pred > 0).sum()}")
        ...
        pred_valid_in_gt_region = ((pred[gt_valid_mask] > 0) & (pred[gt_valid_mask] <= self.max_disparity)).sum()
        density = pred_valid_in_gt_region / total_gt_valid if total_gt_valid > 0 else 0.0
    ...
    def _load_image(self, path, is_gt):
        ...
        if img is None: raise ValueError(f"Failed to load {path}")
