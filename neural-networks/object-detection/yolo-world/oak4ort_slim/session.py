...
def _qnn_ep_devices(ort):
    """Register the plugin EP; return (ep_devices, error_reason).

    Container/NPU setup (FastRPC libs, /dev/fastrpc-cdsp alias,
    ADSP_LIBRARY_PATH) is provided by the onnxruntime oakapp-base image;
    the app only needs the NPU devices passed through in oakapp.toml.
    """
    global _registered
    try:
        import onnxruntime_qnn
        ...
        return (
            [],
            "onnxruntime_qnn is not installed (use the onnxruntime oakapp-base image)",
        )
    if not _registered:
    ...
    if not devs:
        return [], (
            "no NPU EP device was enumerated; make sure the app runs on the "
            "onnxruntime oakapp-base image with /dev/adsprpc-smd and the "
            "dma_heap nodes in optional_devices, and routes through "
            "/entrypoint.sh"
        )
    return devs, ""
